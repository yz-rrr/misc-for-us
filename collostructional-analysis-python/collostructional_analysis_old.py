...
class AssociationStatsKernel:
    ...
    def calculate_all_metrics(
        ...
        lo_stats = cls._calc_log_odds_stats(a, b, c, d)
        fisher_stats = cls._calc_fisher_stats(a, b, c, d, N)
        llr = cls._calc_llr(a, b, c, d, N, expected_a)
        pmi = cls._calc_pmi(a, b, c, d, N)
        dp_stats = cls._calc_delta_p(a, b, c, d)
        kld_stats = cls._calc_kld(a, b, c, d, N)
    ...
    @staticmethod
    def _fisher_exact_r_style(a, b, c, d):
        """
        Calculate using R's fisher.test(..., alternative="two.sided") 
        equivalent logic.
        
        Sums probabilities of all tables with probability <= P_obs.
        This addresses potential discrepancies with SciPy < 1.7 vs R.
        """
        total = a + b + c + d
        
        # scipy.stats.hypergeom(M, n, N) definition:
        # M = total population (Total)
        # n = success states (Row 1 Total: a+b) 
        #     Here we use Col 1 Total (a+c) due to symmetry, 
        #     but align with Draws
        # N = number of draws (Col 1 Total: a+c) 
        #     Alternatively Row 1 (a+b) as Draws is also valid
        
        # Parameter assignment:
        # M = total
        # n = a + c (Target Word Total) [Type I Objects]
        # N = a + b (Construction Total) [Draws]
        # k = a (Observed)
        M = total
        n_good = a + c
        n_draws = a + b
        ...
        # Support range for random variable
        # max(0, Draws - BadItems) <= k <= min(Draws, GoodItems)
        # BadItems = M - n_good
        lower = max(0, n_draws - (M - n_good))
        upper = min(n_draws, n_good)

        # Log probability of observed value
        log_p_obs = hypergeom.logpmf(k_obs, M, n_good, n_draws)

        # Calculate probabilities for full range (vectorized)
        k_range = np.arange(lower, upper + 1)
        all_log_pmfs = hypergeom.logpmf(k_range, M, n_good, n_draws)

        # Apply R-style tolerance (relErr = 1 + 1e-7)
        # log(p) <= log(p_obs * (1 + epsilon)) 
        # -> log(p) <= log(p_obs) + log(1 + epsilon)
        epsilon = 1e-7
        threshold = log_p_obs + np.log(1 + epsilon)

        # Sum probabilities below threshold
        mask = all_log_pmfs <= threshold
        p_val = np.sum(np.exp(all_log_pmfs[mask]))

        # Cap at 1.0 to prevent overflow
        return min(p_val, 1.0)

    @staticmethod
    def _calc_log_odds_stats(a, b, c, d):
    ...
    @staticmethod
    def _calc_fisher_stats(a, b, c, d, N):
        ...
        try:
            ...
            p_val = AssociationStatsKernel._fisher_exact_r_style(a, b, c, d)
            if p_val > 0:
...
class DistinctiveCollexemeAnalyzer(CollexemeAnalyzer):
    ...
    def _handle_two_constructions(
        self, counts, constructions, word_col, total_corpus_size
    ):
