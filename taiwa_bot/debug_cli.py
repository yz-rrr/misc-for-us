...
# ボットの影響度調整用定数（ユーザーの発言の方を重視するなら絶対値を1未満にする）
IMPACT_WEIGHT_BOT = 0.5  # ボット発言の影響度調整用定数
...
async def run_bot_turn_sequence(manager, client, model_name, max_turns=2, last_speaker="User"):
    """
    ボット同士のターン制議論を実行するループ処理
    Returns:
        bool: 議論が収束（終了）した場合は True、継続なら False
    """
    print("\n🤖 **ボット応答 (ターン制):**")
    ...
    for i in range(max_turns):
        # 1. 次の話者を決定
        next_char = manager.select_next_speaker(mode="random", last_speaker=current_speaker)
        role_def = manager.current_roles[next_char]["desc"]
        
        # 2. 文脈取得
        hist_str = "\n".join(manager.history[-10:])
        status_msg = manager.check_convergence()[1] # 現状のステータス取得
        
        # 3. 発言生成
        # 現在のスコアを取得
        current_scores = {name: manager.arguments["main"].scores[name] for name in DEFAULT_ROLES.keys()}
        
        resp = await generate_bot_response(client, model_name, next_char, role_def, manager.topic, hist_str, current_scores, status_msg)
        ...
        # 4. 発言の評価と場の更新
        # ボットの発言も評価に回す
        # ★修正: topic ではなく、現在の論点(arguments["main"].content)を渡す
        current_proposal = manager.arguments["main"].content
        bot_eval_res = evaluate_input(client, model_name, resp, context=hist_str, topic=current_proposal)
        
        if bot_eval_res:
            manager.update_scores(bot_eval_res, impact_weight=IMPACT_WEIGHT_BOT, speaker=next_char)  # ボット発言の影響度は低めに設定
            
            # 収束判定
            status_code, status_msg = manager.check_convergence()
            if status_code in ["AGREED", "REJECTED"]:
                print(f"\n🔴 **ボット間の議論で結論が出ました**: {status_msg}")
                return True # 議論終了
...
async def process_user_input(user_text):
    ...
    eval_res = evaluate_input(client, MODEL_NAME, user_text, 
                              context=recent_history, topic=current_proposal)
    ...
    if eval_res:
        res_detail = manager.update_scores(eval_res, speaker="User")
        status_code, status_msg = manager.check_convergence()
    ...
    # ▼▼▼ 修正: 切り出した関数を呼び出す ▼▼▼
    # ==========================================
    # ユーザーが喋ったので、last_speaker="User" としてボットターンを開始
    debate_ended = await run_bot_turn_sequence(manager, client, MODEL_NAME, max_turns=2, last_speaker="User")

    return debate_ended  # 議論継続
